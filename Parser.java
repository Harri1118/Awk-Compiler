...
public class Parser {
    ...
    public ProgramNode Parse() throws Exception {
        ...
        try{ProgramNode Program = new ProgramNode();
    ...
    public Optional<Node> ParseBottomLevel() throws Exception {
        ...
        if(tokenManager.Peek(0).get().getType() == Token.TokenType.STRINGLITERAL)
            ...
            return Optional.of(new PatternNode());
        ...
        if(!tokenManager.MatchAndRemove(Token.TokenType.ADD).isEmpty()){
            ...
            OperationNode Operation = new OperationNode(Expression, OperationNode.PossibleOperations.PREINC);
            return Optional.of(new AssignmentNode(Expression, Operation));
        }
            // Checks for PREDEC operation
        else if(!tokenManager.MatchAndRemove(Token.TokenType.SUBT).isEmpty()){
            Node Expression = ParseOperation().get();
            OperationNode Operation = new OperationNode(Expression, OperationNode.PossibleOperations.PREDEC);
            return Optional.of(new AssignmentNode(Expression, Operation));
        }
    ...
    private Optional<Node> ParsePost() throws Exception {
        ...
        if(!tokenManager.MatchAndRemove(Token.TokenType.ADD).isEmpty()) {
            ...
            OperationNode Operation = new OperationNode(Variable, OperationNode.PossibleOperations.POSTINC);
            return Optional.of(new AssignmentNode(Variable, Operation));
        }// Check for ADD token, returns PRETINC if so.
        else if(!tokenManager.MatchAndRemove(Token.TokenType.SUBT).isEmpty()) {
            VariableReferenceNode Variable = (VariableReferenceNode) Expression.get();
            OperationNode Operation = new OperationNode(Variable, OperationNode.PossibleOperations.POSTDEC);
            return Optional.of(new AssignmentNode(Variable, Operation));
        }return Expression;
    }
    ...
    public Optional<Node> ParseMatch() throws Exception {
        ...
        if(op.get().getType() == Token.TokenType.TILDE)
            return Optional.of(new OperationNode(leftValue.get(), OperationNode.PossibleOperations.TILDE, n2.get()));
        // If not, return REGEXP
        return Optional.of(new OperationNode(leftValue.get(), OperationNode.PossibleOperations.REGEXP, n2.get()));
    }
    ...
    public Optional<Node> ParseMembership() throws Exception{
        ...
        // n is casted as VariableReferenceNode. This to check if what was parsed was truly an array
        VariableReferenceNode f = (VariableReferenceNode) Array.get();
        if(!f.hasValue())
            throw new Exception("Not an array! Must be a valid array! Must be in the form of EXPRESSION IN ARRAY");
    ...
    public Optional<Node> ParseTernary() throws Exception{
        ...
        Optional<Node> Consequent = ParseOr();
       ...
       if(Consequent.isEmpty())
        ...
        if(tokenManager.MatchAndRemove(Token.TokenType.COLON).isEmpty())
            throw new Exception("Invalid TernaryStatement! Must be in the form of EXP? EXP:EXP");
        Optional<Node> Alternate = ParseOr();
